...
public class MSGraphSyncedUserService extends AADOAuth2UserService {
    ...
    public OidcUser loadUser(OidcUserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        com.example.bgcpromogearreworked.persistence.entities.User updatedUser = userRepo.findByOid(oid).map(user -> {
            user.setDisplayName(graphUser.displayName);
            return user;
        }).orElseGet(() -> {
            com.example.bgcpromogearreworked.persistence.entities.User user = new com.example.bgcpromogearreworked.persistence.entities.User();
